...
import { powerServices, powerProjects, powerHero } from '../data/powerData';
import { PageID } from '../types';
import { PageSidebar, SIDEBAR_CONFIG } from '../components/PageSidebar';

const PowerPage: React.FC<{ onNavigate: (page: PageID, hash?: string, path?: string) => void }> = ({ onNavigate }) => {
  const SectionIcon = powerHero.icon;
  ...
  const sidebarCategory = SIDEBAR_CONFIG.power;
        ...
        <div className="absolute inset-0 flex items-center px-12 lg:px-24 max-w-7xl mx-auto">
          <div className="max-w-3xl">
            ...
            <p className="text-white/75 text-lg leading-relaxed mb-8 max-w-xl">
              {powerHero.pageSubtitle}
            </p>
            <button onClick={() => onNavigate('power', '#services')} className="inline-flex items-center gap-2 bg-brand-accent text-brand-primary px-6 py-3 rounded-lg font-medium hover:bg-white hover:text-brand-primary transition-all duration-200 w-fit">
              Explore Services
      ...
      <nav className="bg-brand-surface border-b border-white/5 px-6 lg:px-24 py-3">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-sm text-brand-muted">
            ...
            <span className="text-brand-foreground font-medium">{powerHero.eyebrow}</span>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-12 lg:px-24 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-2xl font-bold text-brand-foreground mb-2">
              Ready to start your power project?
            </h3>
